    ...
    <section className="space-y-4">
      <header>
        ...
        <p className="text-xs text-muted-foreground">
          {isGuest
            ? 'Purchases are only available after you register or log in with a contact account.'
      ...
      {isGuest && (
        <p className="text-xs text-muted-foreground">
          Guest sessions cannot view purchase history. Please register to see your purchases.
        </p>
      )}
      {isLoading && !isGuest && (
      ...
      {!isGuest && !isLoading && !error && purchases.length === 0 && (
